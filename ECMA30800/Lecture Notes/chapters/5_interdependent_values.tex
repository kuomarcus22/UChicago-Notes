...
Suppose that all others drop out simultaneously. In this case, they must have the same signal. This is the only scenario that makes $b_0^*$ relevant. Let $y \coloneqq y_1=y_2= \cdots = y_N$. 
...
\[
  \mu(s)\coloneqq\exp\left(\int_{0}^{s}\frac{f_Y(x\mid x)}{F_{Y_1}(x\mid x)}\,dx\right).
\]
...
\[
  h(y\mid s)
  \coloneqq \frac{f_{Y_1}(y\mid y)}{F_{Y_1}(y\mid y)}
