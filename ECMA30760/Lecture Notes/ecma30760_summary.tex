...
\begin{proposition}[Quasilinear representation of preferences]
	If $\succsim$ satisfies the axioms, there exists a utility function $u: A \to \R$ such that \[
...
\begin{corollary}
	Pairing this result with the referenced proposition yields that any welfare-maximizing social choice function is implementable in dominant strategies.
\end{corollary}
...
\summarychapter{5}{Social Choice with Indifferences, Constrained Allocation}
...
\begin{corollary}[A revenue-maximizing auction's payments]
	...
	.\] 
	This payment schedule and $X_i(\cdot)$ being nondecreasing (from the referenced proposition) are necessary conditions for an auction to be revenue-maximizing.
...
\summarychapter{7}{Monopolistic Screening}

\summarysection{7.1}{Mussa--Rosen (1978)}
...
\summarychapter{8}{Cooperative Games}

\summarysection{8.2}{Cooperative Games with Transferable Utility}
...
\summarychapter{9}{Stable Matching}

\summarysection{9.1}{One-Sided Matching}
...
\summarysection{9.2}{One-to-One Two-Sided Matching}
...
\summarysection{9.3}{Many-to-One Two-Sided Matching}
...
\summarychapter{10}{Allocation Problems}

\summarysection{10.1}{Deterministic Allocation}

\summarysubsection{Svensson (1999)}
...
\summarychapter{11}{Information Design}

\summarysection{11.2}{General Model}

\begin{lemma}
	 A necessary condition for a distribution over posteriors to have some signal structure that induces it is that the expectation of the posterior beliefs is the prior.
\end{lemma}
...
\begin{definition}[Cooperative game with transferable utility]
	A cooperative game with transferable utility is a pair $(N,v)$, where $N$ is a set of agents and\[
...
\summarysection{8.3}{Cooperative Games with Non-Transferable Utility}
...
\begin{definition}[Responsive]
	A preference $\succ_w$ is responsive if there is a strict ranking $\succ_w^\star$ on $M\cup \left\{w\right\}$ such that for any $S$ with $\left| S \right| < c_w$,\begin{enumerate}[label = (\roman*)]
...
\summarysection{9.4}{More on the Deferred Acceptance Algorithm}
...
\begin{definition}[First-order stochastic dominance]
	...
	We say that $p$ first-order stochastically dominates $p'$ with respect to $\succ_i$ if\begin{itemize}
		\item $p(x_1) \ge p'(x_1)$,
...
\begin{definition}[Strategy-proof, Ex-post efficient, Equal treatment of equals]
	...
	.\]Implicitly, if $f$ is strategy-proof, then the lotteries $f(\succ_i,\succ_{-i})_i$ and $f(\succ_i',\succ_{-i})_i$ must be comparable.
	...
	$f$ satisfies equal treatment of equals (ETOE) if for any $i,j$ and $\succ_{-ij}$, if $\succ_i = \succ_j$, \[
