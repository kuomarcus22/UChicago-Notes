...
The regular FOC/CS conditions hold. The necessary conditions for $\{x^*, u^*\}$ to be an optimum are that the constraint, law of motion, initial condition, and maximization in \ref{eq:9-30-1} hold, where $\lambda$ is as defined.
...
Recall that the current-value Hamiltonian (for $\lambda \coloneqq \Lambda$) is \[
	\hat{H}(x, u, \Lambda) = F(x,u) + \Lambda g(x,u)
...
Linearize around the ODES (\ref{eq:odelambda}, \ref{eq:odek}) to get
\begin{align*}
