...
\begin{example}
	...
	We can see a simple example of the inequalities holding strictly. Let $X_n : \frac{1}{n}$ and $X \coloneqq 0$ almost surely, and let $P_n,P$ denote their distributions. Then $P_n \to P$ weakly since for every bounded continuous $l$,
	\[
...
\begin{proof}
	For (i), consider $f \coloneqq \mathds{1}_{B}$, which is LSC from $B$ being open.
	Then \[
		\liminf\limits_{n \to \infty} P_n(B) = \liminf\limits_{n \to \infty} P_n f \ge P f = P(B)
	.\] For (ii), we can consider the lemma with $g \coloneqq \mathds{1}_{G}$, which is USC from $G$ being closed.
\end{proof}

\begin{notation}[Lower semi-continuous minorant]
	We write \[
		\mathring{f} \coloneqq \sup \left\{g \le f: g \text{ is LSC}\right\} = \sup_{g \in \mathscr{G}} g
	\] and \[
...
\begin{proof}
	...
	We can upgrade this inequality to all of $\mathscr{X}$ from $U$ by fixing some $U$ then seeing \[
		g \coloneqq \left( f(x) -\epsilon \right)\mathds{1}_{U} -(M+1)\mathds{1}_{U^{c}} \le f(y) \le \left( f(x) + \epsilon \right) \mathds{1}_{U} + \left( M+1 \right)\mathds{1}_{U^{c}} =: h
...
\begin{proof}
	...
	With $B \in \mathscr{B}(\mathscr{X})$ and $f \coloneqq \mathds{1}_{B}$, we see that $f$ is discontinuous at $\partial B$. If $P(\partial B) = 0$, then $P (C_f) = 1$, so \[
...
\begin{theorem}[Lindeberg Central Limit Theorem] \label{2-17-0}
	...
	\begin{enumerate}[label=(\roman*)]
		...
		\item (Lindeberg condition) For every $\epsilon > 0$, \[
			L_n(\epsilon) \coloneqq \sum\limits_{i=1}^{k(n)} \E\left[X_{n,i}^2 \mathds{1}_{\left| X_{n,i} \right| > \epsilon}\right] \xrightarrow{n \to \infty} 0
		.\] 
	\end{enumerate}
	Then \[
...
\begin{theorem}[Lindeberg CLT (arbitrary variance)] \label{2-17-3}
	...
	Let
	\[
		S_n^2 \coloneqq \sum\limits_{i=1}^{k(n)} \E\left[X_{n,i}^2\right],
	\]
...
\begin{definition}[Wasserstein distance]
	Suppose we have two distributions $W \sim P$ and $Z \sim Q$. The Wasserstein distance is defined between distributions (but often notated as the distance between random variables) and is \begin{align*}
		\left( \mathscr{W}(W,Z) = \right) \mathscr{W}(P,Q) &\coloneqq \sup_{\operatorname{Lip}(h) \le 1} \left| \E\left[h(W)\right] - \E\left[h(Z)\right]  \right|
	.\end{align*} 
...
\begin{proof}
	...
	\[
		f'(w) - wf(w) = h(w) - Nh
		,\] where $Nh \coloneqq \E\left[h(Z)\right] $ for $Z \sim N(0,1)$.
...
\begin{definition}[Kolmogorov-Smirnov distance]
	...
	\[
		\begin{aligned}
			\operatorname{KS}(P,Q)
			&\coloneqq \sup_t \left| P (-\infty,t] - Q(-\infty,t] \right| \\
