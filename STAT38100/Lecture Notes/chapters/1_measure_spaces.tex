...
\begin{example}[Non-measurable event]
	...
	For every $\lambda$, select an element $x_\lambda \in A_\lambda$ and collect them in the \emph{representative set} $X = \{x_\lambda : \lambda\}$.\footnote{Here we use the Axiom of Choice.} Define the translation \[
...
\begin{definition}[$\sigma$-algebra]
	Let $\Omega$ denote the sample space and $2^{\Omega} \coloneqq \{\text{subsets of } \Omega\}$.
...
\begin{definition}[Monotone sequences of sets and their limits]
	...
	If $A_n \uparrow$, define $\lim\limits_{n \to \infty} A_n \coloneqq \cup_n A_n$, and if $A_n \downarrow$, define $\lim\limits_{n \to \infty} A_n \coloneqq \cap_n A_n$.
\end{definition}
...
\begin{definition}[$\pi$-system, monotone class, $\lambda$-system, field]
	...
	\begin{enumerate}[label=(\roman*)]
		...
		\item If $(A_n)$ is increasing sequence in $\mathscr{L}$, then $\lim\limits_{n \to \infty} A_n \in \mathscr{L}$.\footnote{This assumption is weaker than the one in the definition of a $\sigma$-field.} $\mathscr{L}$ is closed under countable unions, but only for increasing sequences.
	\end{enumerate}
...
\begin{theorem}[Monotone class]
	\boxednote{$m(\mathscr{A})$ is defined to be the smallest monotone class containing $\mathscr{A}$.} If $\mathscr{A}$ is a field, then $m(\mathscr{A}) = \sigma(\mathscr{A})$.
\end{theorem}
...
\begin{proof}
	...
	\[
		M_0 \coloneqq \{A \in m(\mathscr{A}) : A^{c} \in m(\mathscr{A})\}.
	\]
	...
	Define
	\[
		M_1 \coloneqq \{A \in m(\mathscr{A}) : \forall\,B \in \mathscr{A},\; A \cup B \in m(\mathscr{A})\}.
	\]
	...
	Now define
	\[
		M_2 \coloneqq \{A \in m(\mathscr{A}) : \forall\,B \in m(\mathscr{A}),\; A \cup B \in m(\mathscr{A})\}.
	\]
	...
	Finally, let $(A_n)_{n\ge1} \subseteq m(\mathscr{A})$ and set $U_m \coloneqq \bigcup_{n=1}^m A_n$.
...
\begin{remark}[Properties of measure]
	...
	\begin{enumerate}[label=(\roman*)]
		...
		\[
			...
			\qquad\text{where }\ \lim_{n\to\infty}A_n\coloneqq\bigcup_{n=1}^\infty A_n .
		\]
		...
		\[
			...
			\qquad\text{where }\ \lim_{n\to\infty}A_n\coloneqq\bigcap_{n=1}^\infty A_n .
		\]
	\end{enumerate}
\end{remark}
...
\begin{proof}
	\begin{enumerate}[label=(\roman*)]
		...
		\item Assume $A_n \uparrow$ and set $A\coloneqq\bigcup_{n=1}^\infty A_n$. Define
		\[
			D_1 \coloneqq A_1,\qquad D_m \coloneqq A_m \setminus A_{m-1}\ \ (m\ge 2),
		\]
		(with the convention $A_0\coloneqq\varnothing$). Then the $D_m$ are pairwise disjoint and for each $n$,
		\[
		...
		\item Assume $A_n \downarrow$ and $\mu(A_1)<\infty$. Consider $B_n \coloneqq A_1 \setminus A_n$.
...
\begin{theorem}[Restriction to Carath\'eodory-measurable sets]
	Let $\tau$ be an outer measure. Define \[
		\mathscr{F}_\tau \coloneqq \{A \subseteq \Omega: \forall\ B \subseteq  \Omega, \tau(B) = \tau(B \cap A) + \tau(B \cap A^{c})\}
	.\]
...
\begin{proof}
	...
	Defining $\mathscr{F}_\tau$ according to the Carath\'eodory criterion, we know $(\Omega, \mathscr{F}_\tau, \tau)$ is a measure space.
...
\begin{theorem}
	Given an outer measure $\tau$, the measure space $(\Omega, \mathscr{F}_\tau, \tau)$ constructed from the Carath\'eodory criterion is a complete measure space.
\end{theorem}
...
We receive the fact that $\left| \mathscr{B}_{[0,1]} \right| = \left| [0,1] \right|$, which allows us to claim that $\left( [0,1], \mathscr{B}_{[0,1]}, \mu \right)$, where $\mu$ is the uniform measure, is not a complete measure space.\footnote{This implies that the measure space we constructed via Carath\'eodory is strictly larger than this Borel measure space.}
...
What is the connection between the result $\mathscr{F}_\tau$ of Carath\'eodory extension and $\mathscr{B}_{[0,1]}$?
...
\begin{theorem} \label{4-9-1}
	Suppose\boxednote[-7\baselineskip]{One example of a Carath\'eodory completion is taking $\mathscr{A}$ as all finite unions of intervals in the unit interval and $\mathscr{F}_\tau$ as the \emph{completion} of the Borel $\sigma$-field.} $\mu$ is a finite measure on a field $\mathscr{A}$ of subsets of $\Omega$ and that $\tau$ is the outer measure constructed by $\mu$. With $\mathscr{F}_\tau$ as the set of $\tau$-measurable subsets of $\Omega$ according to the Carath\'eodory criterion, $(\Omega, \mathscr{F}_\tau, \tau)$ is the completion of $\left( \Omega, \sigma(\mathscr{A}), \tau \right)$.
\end{theorem}
...
\begin{proof}
	...
	.\] 
	Write $B \coloneqq \cap_m \cup_n B_{m,n} \supseteq A$. Then $B \in \sigma(\mathscr{A})$ because each $B_{m,n} \in \mathscr{A}$.
	We see
	\begin{align*}
		\tau(A) + \frac{1}{n} &\ge \sum_n \mu(B_{m,n}) \\
							  &= \sum_n \tau(B_{m,n}) \\
...
\theoremproofstyle
\begin{proof}[Proof of Theorem~\ref{4-9-1}]
	Write $\mathscr{F} = \sigma(\mathscr{A})$. We want to show that $\tilde{\mathscr{F}} = \mathscr{F}_\tau$, where \[
	...
	Apply the lemma again to $B \setminus A$: there exists $C \in \mathscr{F}$ such that $C \supseteq B \setminus A$ and $\tau(C) = \tau(B \setminus A) = 0$.
