...
\begin{proof}
	...
	Write $f_A(k+1) = \sum_{i \in A} f_i(k+1)$, where $f_i \coloneqq f_{\{i\}}$. Substituting $A = \{i\}$ into the formula and using the Poisson probability $\P[Z = j] = \frac{\lambda^j}{j!}e^{-\lambda}$,
	\begin{align*}
		f_i(k+1)
...
\begin{definition}[Dependency neighborhood]
	For each $i \in \N$, consider $\left\{N_i\right\}$ where \[
		i \in N_i \subseteq [n] \coloneqq \left\{1,2,\ldots,n\right\}
	\] such that \[
...
\begin{example}
	...
	More interestingly, we can count the number of triangles: \[
		T \coloneqq \sum\limits_{1 \le i < j < k \le n}^{} A_{i j}A_{jk}A_{ki}
	.\] 
