...
\begin{definition}[Expectation of a random variable]
	For a random variable $X: \left( \Omega, \mathscr{F}, \P \right) \to \left( \R, \mathscr{B}_\R \right)$, we define the expectation of $X$ as \[
		\E\left[X\right] \coloneqq \int_{}^{} X d\P 
...
\begin{proof}
	...
	Define $g_k(x) \coloneqq \max_{1 \le n \le k} f_{n,k}(x)$. We see that each $g_k$ is a nonnegative simple function. Furthermore, because $f_{n,k} \le f_{n,k+1}$, $\left( g_k \right)$ is increasing\boxednote{$f_{n,k+1}$ is not considered in $g_k$, so the addition of $f_{n+1,k+1}$ poses no problem.}.
	
	We see that $g_k \le f_k$, so $\lim\limits_{k \to \infty} g_k \le \lim\limits_{k \to \infty} f_k = f$.

	Fix $i \in \N$. For every $k \ge i$, since
	\[
		g_k(x) \coloneqq \max_{1 \le n \le k} f_{n,k}(x),
	\]
