...
.\end{align*}
Hence, we get something like $A(t)\mu'' + B(t) \mu' = 0$, which is actually a first-order linear ODE for $z(t) \coloneqq \mu'(t)$.
...
.\end{equation} We will introduce the notion of convergence, where $\mu \coloneqq \limsup\limits_{n \to \infty} \left| a_n \right|^{\frac{1}{n}}$ dictates that if $\left| x-x_0 \right| < \mu^{-1}$, equation (\ref{10-9-1}) converges.
...
\begin{proof}
	...
	\[
	D(t)\coloneqq\int_{t_0}^{t} |\phi(s)-\psi(s)|\,ds ,
	\qquad D(t_0)=0.
	\]
...
\begin{rmk}
	...
	\begin{enumerate}[(i)]
		...
		d(t) \coloneqq \left| \phi(t) - \psi(t) \right| \le L \int_{t_0}^{t} \left| \phi(s) - \psi(s) \right|ds = L \int_{t_0}^{t} d(s) ds  
...
\begin{proof}
	...
	Define
	\[
		\psi(t) \coloneqq \left| x(t) - y(t) \right|,
		\quad
		\alpha(t) \coloneqq \left| x_0 - y_0 \right| + M(t - t_0),
		\quad
		\beta(t) \coloneqq L.
	\]
	Then
...
\begin{proof}
	...
	We  want to show that if $\left| z_0 \right| = \delta \ll 1$, then\marginnote{We can move the factor on $\left| z(t) \right|$ to the right hand side to get the approximate inequality we expect, which will show asymptotic stability.} \[
		d(t) \coloneqq e^{\frac{2}{3} \lambda t} \left| z(t) \right| \le \tilde{C_A} \delta
	.\] 
	...
	\[
	A \coloneqq \nabla F(x^*), \qquad G(z) \coloneqq F(x^*+z) - Az.
	\]
	Then $G(0)=0$ and
	\[
	z'(t) = x'(t) = Az(t) + G(z(t)).
	...
	\end{align*}
	where $\delta\coloneqq\|z_0\|$.

	Let
	\[
	d(t)\coloneqq e^{\frac{2}{3}\lambda t}\|z(t)\|.
	\]
	Then
	\[
...
\begin{proof}
	...
	\[
	  \ell \coloneqq \lim_{t\to\infty} L(x(t))
	\]
	exists.
...
By assumption (ii), $\dot L(x)<0$ for $x\ne x^*$, so in particular
	...
	\[
	  \Delta \coloneqq L(z^*) - L(z(1)) = L(z(0)) - L(z(1)) > 0.
	\]
...
\[
  \omega(D) \coloneqq \bigcup_{z \in D} \omega(z),
\]
...
The Implicit Function Theorem says that for $F \in C^1$, $F(x_0, y_0) = 0$ such that $\partial_y F(x_0,y_0)\ne 0$, we can implicitly define a $C^1$ function $y \coloneqq y(x)$ such that $F(x,y(x))= 0 $ for any $x \in B_\epsilon(x_0)$. 
...
\begin{thm}
	...
	,\] where \[
		M \coloneqq \sup_{(t,x) \in V} \left| f(t,x) - g(t,x) \right|
	.\]  
\end{thm}
...
\begin{proof}
	...
	.\] $\Omega$ is nonempty because it includes $a$, so we can put \[
		t_0 \coloneqq \sup \{t \in \Omega \cap [a,t_1]\}
...
\begin{proof}
	...
	There exists some $N$ such that for all $n \ge N$, $\left| \phi(t_n) - y \right| \le 1$ and $\left| t_n - t_+ \right| \le 1$. On this square \[
		S\coloneqq \left[ t_+ - 1, t_+ +1 \right]\times \left[ y-1,y+1 \right]
...
\begin{defn}[Hyperbolic]
  ...
  \[
	...
	\[
	  Z' = A Z, \qquad A \coloneqq D F(Y^*) = J_F(Y^*),
	\]
