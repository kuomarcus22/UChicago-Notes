...
\[
	\hat{f}_h(x) \coloneqq \frac{1}{nh}\sum\limits_{i=1}^{n} K\left(\frac{X_i-x_0}{h}\right).
\]
...
\begin{enumerate}[label=(\roman*)]
	...
	\item $\sigma^2(x) \coloneqq \operatorname{Var}\left(Y_i \mid X_i = x\right)$ is continuous at $x_0$ and $\sigma^2(x_0) > 0$.
...
Thus $\hat{m}(x_0) \coloneqq \hat{\alpha}=  \sum_{i=1}^{n} \gamma_i Y_i$ where $\gamma_i$ is a function of $X_{1:n}$ and $x_0$.
\end{comment}
