...
\begin{notation}	Let $Z_i \coloneqq (X_i,Y_i)$ with $Z_i \overset{\text{iid}}{\sim} P$ on $\R^d\times \R$.
...
\begin{definition}[Squared-error (prediction) risk]
	...
	\[
		R_P(m) \coloneqq \E_P\big[(Y - m(X))^2\big].
	\]
\end{definition}
...
\begin{remark}	Suppose $X_i \in \{1,\ldots,K\}$ is a categorical random variable. We might estimate by \[
	...
	\begin{enumerate}[label=(\arabic*)]
		...
		\[
			m(k) \coloneqq \E\left[Y\mid X=k\right].
		\]
		With the cell-mean estimator
		\[
			\hat{m}(k) \coloneqq \frac{1}{N_k}\sum\limits_{i: X_i=k}^{} Y_i,
			\qquad
			N_k \coloneqq \#\{i: X_i=k\},
		\]
		...
		\[
			...
			= m(k),
		\]
		so the conditional bias relative to the target $m(k)$ is $0$. In this setting, the only issue is what to do when $N_k=0$. We might define $\hat{m}(k) \coloneqq 0$ in this case, so the bias would be $m(k)$.
		...
		\[
			\hat{m}(x_0) \coloneqq \frac{1}{N_j}\sum\limits_{i: X_i\in I_j}^{} Y_i,
			\qquad
			N_j \coloneqq \#\{i: X_i\in I_j\}.
		\]
		Even if $N_j>0$,
		\[
		...
		\[
			m_j \coloneqq \E\left[Y\mid X\in I_j\right] = \E\left[m(X)\mid X\in I_j\right],
		\]
...
\begin{proof}
	...
	\[
		R_k \coloneqq \left\|X_{(k)}(x_0;\{1,\ldots,n\})-x_0\right\|, 
		\qquad 
		R_{1,j} \coloneqq \left\|X_{(1)}(x_0;I_j)-x_0\right\|
	\]
	...
	Let $m \coloneqq \lvert I_1\rvert$ (so $m \asymp n/(2k)$). We can use the $1$-NN bound at this block size to get
	\[
...
\begin{definition}[H\"older class]
	...
	,\] 
	where $l \coloneqq \left\lfloor \beta \right\rfloor$ is the largest integer strictly less than $\beta.$ In particular, if $\beta \in \N$ then $l = \left\lfloor \beta \right\rfloor = \beta-1$.
\end{definition}
