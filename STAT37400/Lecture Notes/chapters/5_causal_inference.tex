...
.\] 
We will use nonparametrics to estimate the conditional response functions $\mu_{(1)}(x) \coloneqq \E\left[Y_i(1) \mid X_i = x\right] $ and $\mu_{\left(0 \right)}(x) \coloneqq \E\left[Y_i(0) \mid X_i = x\right] $ near the cutoff $c$. We can assume that both $\mu_{(1)}, \mu_{(2)}$ are continuous at $c$,\boxednote{This continuity assumption of $\mu$ at $c$ can be upgraded to something like H\"older continuity.} and as a result, write 
\begin{align*}
	\theta 
...
.\] } \[
	\tau(x) = CATE(x) \coloneqq \E\left[Y_i(1) - Y_i(0) \mid X_i = x\right]  = \mu_{(1)}(x) - \mu_{(0)}(x)
.\] 
...
\begin{notation}	We will now notate
	\[
		e \coloneqq p = \P\left[W_i = 1\right]  \in (0,1)
	.\] 
\end{notation}
...
\begin{definition}[Propensity score]
	Define the propensity score \[
		e(x) \coloneqq \P\left[W_i = 1 \mid X_i=x\right] 
...
\begin{remark}	For intuition,
	think of $\tilde{m}_0(\cdot ), \tilde{m}_1(\cdot ), \tilde{e}(\cdot )$ as fixed functions. Then analogously to as in the $t$-learner (with the new addition of debiasing terms), consider \[
		\tilde{Y}_i(\tilde{m}_1, \tilde{m}_0, \tilde{e}) \coloneqq \tilde{m}_1\left( X_i \right) - \tilde{m}_0(X_i) + \frac{W_i \left( Y_i - \tilde{m}_1(X_i) \right)}{\tilde{e}(X_i)} - \frac{(1-W_i)(Y_i  - \tilde{m}_0(X_i)}{1-\tilde{e}(X_i)}
	.\] 
	...
	Suppose $\tilde{m}_1 = m_1, \tilde{m}_0 = m_0$, but $\tilde{e}$ is arbitrary. Then
		\[
			A_i \coloneqq m_1(X_i) + \frac{W_i (Y_i - m_1(X_i))}{\tilde{e}(X_i)},
			\qquad
			B_i \coloneqq m_0(X_i) + \frac{(1-W_i)(Y_i - m_0(X_i))}{1- \tilde{e}(X_i)}.
		\]
	...
	Then 
		\[
			\tilde{A}_i \coloneqq \tilde{m}_1(X_i) + \frac{W_i (Y_i - \tilde{m}_1(X_i))}{e(X_i)},
			\qquad
			\tilde{B}_i \coloneqq \tilde{m}_0(X_i) + \frac{(1-W_i)(Y_i - \tilde{m}_0(X_i))}{1- e(X_i)}.
		\]
