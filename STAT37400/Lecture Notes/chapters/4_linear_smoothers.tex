...
\[
Q(\theta)
:= \sum\limits_{i=1}^{n} w_i \left( Y_i - \alpha - \beta(X_i-x_0) \right)^2
= r(\theta)'W r(\theta)
...
\[
\gamma'
:= e_1'(\tilde{X}'W\tilde{X})^{-1}\tilde{X}'W,
\qquad\text{so that}\qquad
\hat{m}(x_0)=\sum\limits_{i=1}^{n}\gamma_i Y_i.
\]

Equivalently, writing $S_k := \sum\limits_{j=1}^{n} w_j (X_j-x_0)^k$ for $k=0,1,2$,
the denominator
\[
D := S_0S_2 - S_1^2
\]
...
\end{corollary}
\begin{proof}
	Formally, define $\tilde{Y}_i := Y_i - m'(x_0)(X_i-x_0)$, and let $m(x) := \E\left[Y_i \mid X_i = x\right]$ and $\tilde{m}(x) := \E\left[\tilde{Y}_i \mid X_i = x\right]$. Then 
	\begin{align*}
...
Recall that with Nadaraya-Watson, we said that we could change the kernel near and at the boundary. Given a linear smoother $\hat{m}(x_0)=\sum\limits_{i=1}^{n} \gamma_i Y_i$ with $\sum\limits_{i=1}^{n}\gamma_i=1$, we can always write it in Nadaraya-Watson form by defining data-dependent weights $\tilde{K}_i := \gamma_i$:
\[
...
\[
\mathscr{G}(2,C) := \left\{m: \sup_{x\in[0,1]} \left| m''(x) \right| \le C\right\}
.\]
...
\[
	\begin{aligned}
		...
		&\le B(x_0) := \sup_{m \in \mathscr{M}} \left|\sum\limits_{i=1}^{n} \gamma_i m(X_i) - m(x_0)\right|
	\end{aligned}
.\]
...
\[
	\tilde{b} := \frac{B(x_0)}{\operatorname{se}\left(\hat{m}(x_0) \mid X_{1:n}\right)}
	\quad\text{and}\quad
	\chi(\tilde{b}) := \inf\left\{t: \inf_{|b|\le \tilde{b}} \P\left[\left|Z+b\right|\le t\right] \ge 1-\alpha\right\}
.\]
Then
\[
...
Define
\[
	\mu_i := \E\left[Z_i\right], \quad \tau_i^2 := \operatorname{Var}\left(Z_i\right), \quad \rho_i := \E\left[\left|Z_i-\mu_i\right|^3\right]
.\] 
We know \[
...
\end{theorem}
\begin{example}Consider the case where $Z_i$ are i.i.d. with $\rho:=\E\left[\left|Z_1-\E\left[Z_1\right]\right|^3\right]$ and $\tau^2 := \operatorname{Var}\left(Z_1\right)$. Then the bound becomes
\[
...
\[
	Z_i := \gamma_i\left(Y_i - m(X_i)\right),
	\quad \mu_i = 0,
	\quad
	\begin{aligned}
