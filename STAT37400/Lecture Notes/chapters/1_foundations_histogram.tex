...
\begin{example}[Common functionals (one-dimensional $Y$)]
	...
	\begin{enumerate}
		...
		\[
			\theta(P)=F(y)\coloneqq\P_P(Y_i\le y)=\E_P[\mathbf 1\{Y_i\le y\}].
		\]
...
\begin{definition}[Squared-error risk / MSE]
	...
	\[
		R_P(\hat\theta,\theta)\coloneqq\E_P\big[(\hat\theta-\theta(P))^2\big],
	\]
...
\begin{example}[Sample mean]
	Let $\theta(P)=\E_P[Y_i]$ and consider $\hat\theta=\bar Y\coloneqq\frac1n\sum_{i=1}^n Y_i$.
...
\begin{example}
	...
	Define
	\[
		\hat F_n(y)\coloneqq\frac{1}{n}\sum_{i=1}^n \mathds{1}_{Y_i\le y}.
	\]
...
\begin{remark}[Variance increases as $h$ decreases]
	Let $p_j\coloneqq\P_P(Y_i\in I_j)$. Since $\mathbf 1\{Y_i\in I_j\}\sim \mathrm{Bernoulli}(p_j)$,
	\[
...
\begin{definition}[Histogram estimator at $y_0$]
	Partition $[0,1]$ into $K$ intervals (bins) $I_1,\dots,I_K$ of equal length $h\coloneqq1/K$.
...
\begin{proof}
	Let $p_j\coloneqq\P_P(Y_i\in I_j)=\int_{I_j} f(y)\,dy\le Mh$. Since $\mathbf 1\{Y_i\in I_j\}\sim\mathrm{Bernoulli}(p_j)$,
	\[
		\Var_P(\hat f_h(y_0))
		=
...
\begin{remark}	Formally, $f = F'$. One might try to estimate $f(y_0)$ by differentiating the empirical CDF
	\[
		\hat F_n(y)\coloneqq\frac1n\sum_{i=1}^n \mathbf 1\{Y_i\le y\}.
	\]
...
\begin{remark}[Classical normal-approximation heuristic]
	...
	\[
		...
		\qquad
		\mathrm{se}(\hat\theta)\coloneqq\sqrt{\Var(\hat\theta)},
	\]
