...
To specify a (time homogeneous) Markov process, we only need to describe the distribution of $X_0$ and the transition probabilities $\P\left[ x,y \right] \coloneqq P\left[ X_1 = y \mid X_0 = x \right].$
...
\begin{proof}
Write $\mathbb{P}_x(\,\cdot\,)\coloneqq\mathbb{P}(\,\cdot\,\mid X_0=x)$, so that
...
\begin{defn}[Transition matrix]
	...
	Put $\pi_j = \P\left[ X_0 = j \right]$, so $\pi \coloneqq \left( \pi_1,\ldots,\pi_N \right)$ is a row vector encoding the starting probabilities.
\end{defn}
...
\begin{proof}
	...
	Let $y \in S$. Since the Markov chain is irreducible, there exists $n \in \N$ such that $p^n(x,y) > 0$. So, there exists $k$ such that $\P\left[y \in \{X_{\tau_k}, \ldots,X_{\tau_{k+1}}\}\right] > 0$. Because these tuples (sets) are i.i.d, the probabilities do not depend on the choice of $k$, so we can put \[
		q \coloneqq \P\left[y \in \{X_{\tau_k},\ldots, X_{\tau_{k+1}}\}\right] 
	.\] 
...
\begin{ex}
	...
	hitting time
	\[
		\tau \coloneqq \min\{ n \ge 0 : X_n = -a \text{ or } X_n = b \}
	.\] 
	...
	Write
	\[
		p \coloneqq \P\left[ X_\tau = b \right]
	\]
...
\begin{ex}
	...
	Consider the stopped chain $Y_n \coloneqq X_{\tau \wedge n}$. Then $\{Y_n\}$ is a
	...
	On the other hand $X_\tau \in \{-a,b\}$, so with $p \coloneqq \P\left[ X_\tau = b \right]$
	we have
	\[
...
\begin{thm}
	...
	Then
	\[
		\P\left[ M_\infty \coloneqq \lim_{n \to \infty} M_n \text{ exists} \right] = 1
	.\] 
\end{thm}
...
\begin{ex}
	...
	\[
		X_\infty \coloneqq \lim_{n \to \infty} X_{n \wedge \tau} \quad \text{a.s.}
	\]
...
\begin{ex}
	...
	\begin{enumerate}[(i)]
		\item Let $\{Y_k\}_{k \ge 1}$ be iid with
		      ...
		      \[
			      M_n \coloneqq \sum_{k=1}^{n} k Y_k
		      \]
		      with the natural filtration $\mathscr{F}_n \coloneqq \sigma(Y_1,\ldots,Y_n)$.
...
Harmonic functions solve the heat equation, as we just put $f(\overline{x},t) \coloneqq f(\overline{x})$.
...
\begin{proof}
	...
	\begin{align*}
		\partial_t \E\left[f(\overline{B}_t + \overline{x}_0 , t)\right] &= \partial_t \int_{\R^d}^{} f(\overline{x}+\overline{x}_0, t) p_t(\overline{x})dx_1\cdots dx_d  \\
				 ...
				 &= \int_{\R^d}^{} \partial_t p_t(\overline{x}-\overline{x}_0) f(\overline{x},t) + p_t(\overline{x},\overline{x}_0) \partial_t f(\overline{x}, t)dx_1\cdots dx_d \\
				 &= \int_{\R^d}^{} \frac{1}{2} \Delta p_t(\overline{x}-x_0) f(\overline{x},t) + p_t(\overline{x}-\overline{x}_0) \partial_t f(\overline{x}, t) dx_1 \cdots dx_d
	.\end{align*}
	We can integrate the first term by parts so the extra terms decay very quickly according to our assumption on $f$.
	\begin{align*}
		\partial_t \E\left[f(\overline{B}_t + \overline{x}_0 , t)\right] &= \int_{\R^d}^{} p_t (\overline{x} - \overline{x}_0) \left( \frac{1}{2}\Delta f(\overline{x},t)  + \partial_t f(\overline{x},t) \right)dx_1\cdots dx_d = 0	
	,\end{align*}
