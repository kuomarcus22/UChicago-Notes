...
\begin{theorem}	Suppose $\E\left[y^2\right]<\infty$ and $\E\left[x_j^2\right]<\infty$ for each $j=1,\ldots,k$. The function $g(x)\coloneqq\E\left[y\mid x\right]$ is the best predictor of $y$ given $x$ under square loss. That is:
	\[
...
\begin{definition}[Residual Maker Matrix]
	...
	\]
	Therefore, the matrix $M_X\coloneqq I_n-X(X'X)^{-1}X' = I_n - P_X$ is called the residual maker.
...
\begin{definition}[$R^2$]
	\begin{itemize}
		\item $\sum_{i=1}^n(y_i-\bar y)^2\coloneqq\text{TSS}$ is called the total sum of squares.
		\item $\sum_{i=1}^n(y_i-x_i'\hat\beta)^2\coloneqq\text{SSR}$ is the sum of squared residuals.
		\item $\sum_{i=1}^n(\hat y_i-\bar y)^2\coloneqq\text{ESS}$ is the explained sum of squares.
	\end{itemize}
...
\begin{remark}	High $R^2$ does not mean the linear model is ``correct'' in the sense that it provides a causal explanation of the relationship between $x$ and $y$.
	...
	\[
		\operatorname{Var}(y)\coloneqq\E\left[\operatorname{Var}(y\mid x)\right]+\operatorname{Var}(\E\left[y\mid x\right]).
	,\]
...
\begin{proof}
	...
	\[
		...
		\] The off diagonal elements are $0$ because of MLR.2 and MLR.4,\footnote{MLR.2 gives independence across observations, so conditional on $x_i,x_j$ we can separate the two errors; MLR.4 then gives $\E[u_j\mid x_j]=0$. MLR.5 only determines the conditional variance on the diagonal.} \emph{not} because of MLR.5.
...
\begin{proof}(Gauss-Markov Theorem)
	...
	From now on, write $A \coloneqq A\left( X \right)$.
...
\begin{proposition}[Frisch--Waugh--Lovell in sample form]
	...
	\[
		...
		= (X_1^{*'}X_1^*)^{-1} X_1^{*'} Y^*
	\]
	where $M_{X_2} \coloneqq I_n - P_{X_2}$, $X_1^* \coloneqq M_{X_2}X_1$ and $Y^* \coloneqq M_{X_2}Y$.
...
\begin{proposition}[OLS formula for a subset of coefficients]
	...
	\]
	Equivalently, letting $X_2^* \coloneqq M_{X_1}X_2$ and $Y^* \coloneqq M_{X_1}Y$, we have
	\[
...
\begin{remark}[A single coefficient as a regression on residuals]
	...
	Writing $r \coloneqq M_{X_1}x_j$, we have
	\[
...
\begin{proposition}[Variance of a subset of coefficients]
	...
	\]
	where $SSR_j \coloneqq \sum_{i=1}^n r_i^2$ is the SSR from regressing $x_j$ on the other regressors (including a constant).
	Writing $SST_j\coloneqq\sum_{i=1}^n (x_{ij}-\bar{x}_j)^2$ and denoting the corresponding $R^2$ by $R_j^2$, we have
	\[
...
\end{proposition}
\begin{proof}
	Because $(b_0,b_1)$ defines the best linear predictor of $y$ given $(1,x_1)$, the projection residual $v\coloneqq y-b_0-b_1x_1$ satisfies
	\[
		\E[v]=0
		\qquad \text{and} \qquad
		\E[v x_1]=0.
	...
	Can we show analytically why this is the case?
		...
		\[
			\begin{aligned}
				y &\coloneqq \ln(\text{price}) \in \R^{n\times 1}, \\
				X_4 &\coloneqq \text{bdrms} \in \R^{n\times 1}, \\
				X_5 &\coloneqq \ln(\text{sqrft}) \in \R^{n\times 1}, \\
				c &\coloneqq \mathbf{1}_n \in \R^{n\times 1}.
			\end{aligned}
		\]
		and define the ``other columns'' in the long regression by
		\[
			X_3 \coloneqq [\,c \mid X_5\,] \in \R^{n\times 2}.
		\]
	...
	We have
	\[
		M_c \coloneqq I_n - P_c, \qquad P_c \coloneqq c(c'c)^{-1}c' = \frac{1}{n}cc'.
	\]
	...
	\[
		\hat{\delta}_1 \coloneqq (X_4' M_c X_4)^{-1} X_4' M_c X_5,
	\]
	...
	\[
		\bar{x}_4 \coloneqq \frac{1}{n}c'X_4, \qquad \bar{x}_5 \coloneqq \frac{1}{n}c'X_5.
	\]
